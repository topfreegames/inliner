...
function Inliner(source, options, callback) {
  ...
  if (options.loadedFiles) {
    this.loadedFiles = options.loadedFiles;
  } else {
    this.loadedFiles = {};
  }
  if (options.filename) {
...
function main() {
  ...
  .then(function exists(isFile) {
    ...
    return inliner.get(this.filename ||
       url, { encoding: 'binary', loadedFiles: this.loadedFiles  });
  })
  .catch(function isUrl() {
    ...
    if (inliner.url || url.indexOf('<') === -1) {
      ...
      debug('inlining url');
      return inliner
      .get(url, { encoding: 'binary', loadedFiles: this.loadedFiles  });
    }
